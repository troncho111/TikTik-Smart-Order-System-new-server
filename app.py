...
from auth_helpers import restore_session_from_token, clear_session_token, set_session_token, set_session_token
...
section[data-testid="stSidebar"] body {
...
# Global Hard Reset Button in Sidebar
with st.sidebar:
    st.markdown("---")
    st.warning("⚠️ אזור ניהול")
    if st.button("🏠 דף הבית (הזמנה חדשה)", type="primary", use_container_width=True, key="hard_reset_final_v2"):
        # Total Wipeout - Absolutely nothing remains
...
def show_selection_summary():
    ...
    with col1:
        st.info(f"📋 **בחירה:*body {product_label}  |  {event_label}")
        ...
        if st.session_state['saved_games']:
            ...
            for idx, saved_game in enumerate(saved_list):
                ...
                if len(saved_list) >= 2:
                    ...
                    st.success(f"**{idx + 1}.*body {game_text}")
                    if details:
                        st.caption(details)
    ...
    with col2:
        ...
        if saved_games_summary:
            ...
            for idx, sg in enumerate(saved_games_summary):
                ...
                det = sg.get('details', '')
                st.write(f"- **אירוע {idx + 1}:*body {txt}")
                if det:
                    st.caption(det)
            if event_date:
                st.write(f"**תאריך (אירוע ראשון):*body {event_date.strftime('%d/%m/%Y')} {event_time.strftime('%H:%M')}")
            if venue:
                st.write(f"**מקום:*body {venue}")
        else:
            if event_name:
                st.info(f"**אירוע:*body {event_name}")
            if event_date:
                st.write(f"**תאריך:*body {event_date.strftime('%d/%m/%Y')} {event_time.strftime('%H:%M')}")
            if venue:
                st.write(f"**מקום:*body {venue}")
        if customer_name:
            st.write(f"**לקוח:*body {customer_name}")
        if category:
            st.write(f"**קטגוריה:*body {category}")
        ...
        if total_foreign > 0:
            st.success(f"**סה\"כ:*body {total_foreign} {currency_symbol} = {total_nis:,} ש\"ח")
        ...
        for proposal in filtered_proposals:
            ...
            with st.container():
                ...
                with col1:
                    ...
                    st.markdown(f"👤 **לקוח:*body {prop_dict.get('customer_name', '')}")
...
def page_order_summary():
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.write(f"**👤 לקוח:*body {order_data.get('customer_name')}")
        st.write(f"**📅 תאריך:*body {order_data.get('event_date')}")
        st.write(f"**📍 מיקום:*body {order_data.get('venue')}")
    with col2:
        st.write(f"**🎫 כרטיסים:*body {order_data.get('num_tickets')}")
    ...
    with st.expander("👤 פרטי לקוח", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            st.write(f"**שם:*body {order_data.get('customer_name', '—')}")
            st.write(f"**ת.ז.:*body {order_data.get('customer_id') or '—'}")
        with c2:
            st.write(f"**טלפון:*body {order_data.get('customer_phone') or '—'}")
            st.write(f"**אימייל:*body {order_data.get('customer_email') or '—'}")

    # --- כרטיס: אירוע ראשי ---
    with st.expander("🎫 אירוע ראשי", expanded=True):
        st.write(f"**אירוע:*body {order_data.get('event_name', '—')}")
        st.write(f"**תאריך ושעה:*body {order_data.get('event_date', '—')}")
        st.write(f"**מיקום:*body {order_data.get('venue') or order_data.get('venue_name') or '—'}")
        if order_data.get('category'):
            st.write(f"**קטגוריה:*body {order_data.get('category')}")
        if order_data.get('ticket_description'):
            st.write(f"**פרטי כרטיס:*body {order_data.get('ticket_description')[:200]}{'…' if len(order_data.get('ticket_description', '')) > 200 else ''}")
    ...
    if order_data.get('product_type') == 'package' and order_data.get('hotel_name'):
        with st.expander("🏨 מלון", expanded=True):
            st.write(f"**מלון:*body {order_data.get('hotel_name')}")
            if order_data.get('hotel_nights'):
                st.write(f"**לילות:*body {order_data.get('hotel_nights')}")
            if order_data.get('hotel_stars'):
                st.write(f"**כוכבים:*body {order_data.get('hotel_stars')}")
            if order_data.get('hotel_meals'):
                st.write(f"**ארוחות:*body {order_data.get('hotel_meals')}")
            if order_data.get('hotel_address'):
                st.write(f"**כתובת:*body {order_data.get('hotel_address')}")
    ...
    if flights:
        with st.expander(f"✈️ טיסות ({len(flights)})", expanded=True):
            for f in flights:
                ...
                st.write(f"**{direction}:*body {fr} → {to}" + (f" ({airline})" if airline else ""))
    ...
    with st.expander("💰 סיכום תמחיר", expanded=True):
        ...
        with p1:
            st.write(f"**מחיר ליחידה:*body {order_data.get('currency_symbol', '€')}{order_data.get('price_per_ticket') or 0:,.0f}")
        with p2:
