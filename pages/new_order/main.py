...
from ui_helpers import get_random_atmosphere_image, render_header
...
def page_new_order():
    ...
    # Global Home / Reset Button
    if st.sidebar.button("🏠 דף הבית (הזמנה חדשה)", type="primary", use_container_width=True):
        for key in list(st.session_state.keys()):
            if key not in ['user', 'authenticated', 'page']:
    ...
    with col1:
        ...
        if product_type == "package":
            ...
            if lookup_hotel and hotel_name:
                # Get venue/city from saved games for better hotel search
                _venue = ''
                if st.session_state.get('saved_games'):
                    _first = st.session_state['saved_games'][0]
                    _venue = _first.get('worldcup_venue') or ''
                    if not _venue and _first.get('fixture_data'):
                        _fi = _first['fixture_data']
                        _venue = (_fi.get('venue') or '') + (f", {_fi.get('city')}" if _fi.get('city') else '')
                    if not _venue and _first.get('concert_venue_name'):
                        _venue = _first.get('concert_venue_name', '') + (f", {_first.get('concert_venue_city')}" if _first.get('concert_venue_city') else '')
                city = _venue.split(',')[-1].strip() if _venue and ',' in _venue else ''
            ...
            elif scan_flights_btn and not flight_image_to_scan:
                st.warning("⚠️ יש להעלות או להדביק צילום מסך של טיסות לפני הסריקה")
            ...
            # Initialize with detected value or from saved data (don't pass value= if key in session_state - avoids Streamlit warning)
            default_outbound_airline = detected_outbound_airline or fd['outbound'].get('airline', '')
            _kwargs = {"placeholder": "Air Europa", "key": "flight_outbound_airline"}
            if "flight_outbound_airline" not in st.session_state:
                _kwargs["value"] = default_outbound_airline
            outbound_airline = st.text_input("חברת תעופה", **_kwargs)
            ...
            _kwargs_ret = {"placeholder": "El Al", "key": "flight_return_airline"}
            if "flight_return_airline" not in st.session_state:
                _kwargs_ret["value"] = default_return_airline
            return_airline = st.text_input("חברת תעופה", **_kwargs_ret)
        ...
        customer_name = st.text_input("שם מלא", value=st.session_state.random_data.get('customer_name', ''), placeholder="ישראל ישראלי", key="cust_name_input")
        st.session_state.random_data['customer_name'] = customer_name
        
        col_id, col_phone = st.columns(2)
        with col_id:
            customer_id = st.text_input("תעודת זהות", value=st.session_state.random_data.get('customer_id', ''), placeholder="123456789", key="cust_id_input")
            st.session_state.random_data['customer_id'] = customer_id
        with col_phone:
            customer_phone = st.text_input("טלפון", value=st.session_state.random_data.get('customer_phone', ''), placeholder="050-1234567", key="cust_phone_input")
            st.session_state.random_data['customer_phone'] = customer_phone
        
        customer_email = st.text_input("אימייל", value=st.session_state.random_data.get('customer_email', ''), placeholder="example@email.com", key="cust_email_input")
        st.session_state.random_data['customer_email'] = customer_email
    ...
    with col2:
        ...
        st.markdown("""
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 10px; border-right: 5px solid #667eea; margin-bottom: 20px;">
            <h4 style="margin: 0; color: #2c3e50;">📋 סיכום מהיר</h4>
        </div>
        """, unsafe_allow_html=True)
        ...
        if saved_games_summary:
            for idx, sg in enumerate(saved_games_summary):
                st.info(f"**{idx + 1}. {sg.get('display_text', 'אירוע')}**")
            if event_date:
        ...
        if total_foreign > 0:
            st.markdown(f"""
            <div style="background-color: #e3f2fd; padding: 15px; border-radius: 10px; border-right: 5px solid #2196f3;">
                <h4 style="margin: 0;">💵 סה"כ: {total_foreign} {currency_symbol} = {total_nis:,} ש"ח</h4>
            </div>
            """, unsafe_allow_html=True)
        ...
        # Validation checks
        errors = []
        if not event_name and not saved_games_summary: errors.append("שם אירוע")
        if not customer_name: errors.append("שם לקוח")
        if not customer_email: errors.append("אימייל")
        if not category: errors.append("קטגוריה (בלוק)")
