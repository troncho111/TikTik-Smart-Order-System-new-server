...
def show_product_selection():
    ...
    if st.sidebar.button("🏠 דף הבית (הזמנה חדשה)", type="primary", key="home_reset_step1", use_container_width=True):
...
def show_event_selection():
    ...
    # Global Home / Reset Button
    if st.sidebar.button("🏠 דף הבית (הזמנה חדשה)", type="primary", key="home_reset_step2", use_container_width=True):
        st.session_state.clear()
        st.rerun()
...
def show_selection_summary():
    ...
    if event_type == "body": event_type = None
    ...
    event_label = event_labels.get(event_type, 'לא נבחר')
    ...
    with col2:
        if st.button("🔄 שנה בחירה", type="secondary", use_container_width=True, key="reset_selection"):
            for k in ['ui_step', 'product_type_selected', 'event_type_selected', 'saved_games']:
